...
```{r}
coef(res_fit.ridge,  s=res_bestlam.ridge)
```
...
These are the coefficients:
```{r}
res_bestlam.lasso.coef <- coef(res_fit.lasso,  s=res_bestlam.lasso)

res_bestlam.lasso.coef[res_bestlam.lasso.coef[,1]!=0,]
```
...
These are the coefficients:
```{r}
coef(eme_fit.ridge,  s=eme_bestlam.ridge)
```
...
These are the coefficients:
```{r}
eme_bestlam.lasso.coef <- coef(eme_fit.lasso,  s=eme_bestlam.lasso)

eme_bestlam.lasso.coef[eme_bestlam.lasso.coef[,1]!=0,]
```

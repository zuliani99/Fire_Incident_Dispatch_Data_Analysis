...
# Cast to a ClassificationTtask

As mentioned before, we decided, as the professor suggested, to cast our regression problem into a classification problem by dividing the range of possible time difference responses into 2 for a binary classification or more than 2 for a multi-classification task.
...
As expected we have an unbalanced case, since exactly 3/4 of the train and test dataset have a fast response, whereas the rest has a low response.
...
- **inc_borough** is strongly significant for Staten Island and Manhattan and heavily significant for Queens.
- **cong_dist** is strongly significant for the congressional district 5 and 12, whereas it is slightly significant for 14 and 16.
- **engined_assigned** and **others_units_assigned** are strongly significant but not **ladders_assigned**.
- Interestingly the **inc_class_group** is strongly significant only for the structural and non structural fires.
...
And update the model removing the predictors that are explained with a linear combination of others predictors. We name this new model the cleaned one since it will contain only the significant predictors.
...
- `al_source_desc` between `others_units_assigned ` is significant for many cases but for others like.
...
We can see that in the case of having more than a single units assigned we have an higher probability in the Evening of having a fast response, similarly n the case of having a single assigned units. Interestingly we can see that the two line plots are pretty similar, they are only vertically scaled.
...
However we recall that we have an unbalanced case and for this reason model choice should happen on the one that is able to correctly classify the response time of an incident, thus we choose the **Full Model** since it has the highest sensitivity and specificity is not soo bad.
...
We recall that the penalty on ridge is put in the sum of square of the coefficients thus to shrinkage the model coefficient toward zero by the parameter $\lambda$.
...
In Lasso instead of the sum of square of the coefficients we penalize the sum of absolute value of the coefficients.
...
The remaining non zero coefficient from the model obtained with the lambda taken 1se after are:
```{r}
res_lasso.1se.coef <- coef(res_fit.lasso, res_bestlam.lasso)

res_lasso.1se.coef[res_lasso.1se.coef[,1]!=0,]
```
...
```{r}
with(cl_resp_min_fd.train, 
     mean(time_of_day[fast_response == FALSE] == "Evening"))
```
...
However recalling the unbalanced problem our choice could be assigned to both:

- **Full Model** since it has the highest accuracy at equal merit with the Naive Bayes, it is second overall regarding the sensitivity even if is the penultimate on specificity but it is not worse than random choice.
- **Naive Bayes** since has the same accuracy but it has the best sensitivity and worst specificity.
...
- **cong_dist** is highly significant only for the congressional district 13
...
Now we remove already the non significant predictor building as before the cleaned model.
...
And it seems like that the full model with the interactionit is just slightly better respect to the full model without interaction terms.


## Model Interpretation

We decide to interpret the model with the lower AIC, so the **eme_glm.full.inter** which has ```r AIC(eme_glm.full.inter)``` as AIC.
...
The lower probability of having a fast emergency is for the borough of Brooklyn, whereas the lower probability is for both Bronx and Manhattan having also a pretty big confidence interval.
...
```{r}
plot(effect("others_units_assigned", eme_glm.full.inter), rescale.axis = FALSE, 
     ylab = "Probability of Fast Emergency")
```
...
The remaining non zero coefficient from the model obtained with the lambda taken 1se after are:
```{r}
eme_lasso.1se.coef <- coef(eme_fit.lasso, eme_bestlam.lasso)

eme_lasso.1se.coef[eme_lasso.1se.coef[,1]!=0,]
```
...
In again more or less we have similar results for all the model, however differently from the previous analysis this time we choose as best overall model the **Cleaned Model**.

...
summary(lm_irm_full)
```
By the summary we can see that the model $R^2$ tell us that we are able explain around the $12.83\%$ percent of the total variability of the data and also the Adjusted R-squared which penalise the complexity of our model tell us the same story.
...
1. **Residuals vs Fitted Plot**: Used to check the linear relationship assumptions, this is proved by the straight horizontal red line at zero level. Even if, we have an higher amount of spreaded observations on the top right of the red line.
2. **Q-Q Residuals Plot**: in this plot called also quantile - quantile residual plot and tells us if the residuals are normally distributed or not. If they follows the dotted red line we can say so otherwise as in our case we can't say that are normally distributed, as we will see in much detail later.
...
4. **Residuals VS Leverage plot**: helps us to identify the outliers and high levarage points with the Cook's distance, so points that have influence on the regression line.
In this plot we look for data points outside of a dashed line, Cook’s distance. When the points are outside of the Cook’s distance, this means that they have high Cook’s distance scores. In our case we do not have any observations that satisfy what we have just saied
...
Like the previous model we can say the residuals follow a linear pattern but this time better that the previous model, even if the residual are not randomly spreaded since they are again on the right. And it seem like the left observation the Scale - Location plot brings up the variabilit of residual, where as the it is constant inside the cluster. Again we do not have any influential point. But on the other hand the qqPlot is pretty much a mess indicating that the residuals are not normally distributed, by this qqPlot we can say that:
...
```{r}
lm_irm_full_upd_2 <- update(lm_irm_full_upd, . ~ . - others_units_assigned - engines_assigned - ladders_assigned + log(engines_assigned + 1) + log(ladders_assigned + 1) + log(others_units_assigned + 1) + log(engines_assigned + 1))
...
1. **Residuals vs Fitted plot**: here we can see that the residuals have a linear pattern, and the residuals seems pretty much randomly spreaded.
...
summ_lm_em_full <- summary(lm_em_full)
summ_lm_em_full
```

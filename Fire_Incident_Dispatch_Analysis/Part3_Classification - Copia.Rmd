...
As expected we found **multicollinearity** problem so let's the `VIF`

```{r}
vif(res_glm.fit_full)
```

```{r}
res_glm.fit_full_upd <- update(res_glm.fit_full, . ~ . - cong_dist - highest_al_level - inc_class_group + al_source_desc : engines_assigned + al_source_desc : others_units_assigned + al_source_desc : ladders_assigned, data = cl_resp_min_fd.train)
...
Before continuing let's understand the effects of each coefficients respect to the probability of having a fast response. We use the function `allEffects` to inspects these behaviours.
We analyse some predictors since like the one that have a high statistical significant p-value. 
```{r}
plot(effect("inc_borough", res_glm.fit_full),rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("inc_class_group", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("engines_assigned", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("day_type", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("working_hour", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("time_of_day", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
plot(effect("tua_is_one", res_glm.fit_full), rescale.axis = FALSE, ylab = "Probability of Fast Response")
```
...
```{r}
res_glm.fit_full_upd <- update(res_glm.fit_full, . ~ . - cong_dist - ladders_assigned + al_source_desc : engines_assigned + al_source_desc : others_units_assigned + al_source_desc : ladders_assigned, data = cl_resp_min_fd.train)
summary(res_glm.fit_full_upd)
```
